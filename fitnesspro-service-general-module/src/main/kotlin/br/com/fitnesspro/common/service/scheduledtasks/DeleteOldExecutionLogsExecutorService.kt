...
class DeleteOldExecutionLogsExecutorService(
    ...
    private val executionLogsSubPackageRepository: IExecutionsLogSupPackageRepository,
    ...
    override fun execute(config: DeleteOldExecutionLogsConfig?, pairIds: Pair<String, String>) {
        ...
        val executionLogsSubPackages = executionLogsPackages.map {
            customDeleteExecutionsLogRepository.getIdsExecutionLogSubPackageDelete(it)
        }

        executionLogsSubPackages.flatten().chunked(60000).forEach(executionLogsSubPackageRepository::deleteAllByIdInBatch)
        ...
        val additionalInformation = StringJoiner("\n").apply {
            ...
            add(" ${executionLogsSubPackages.size} registros de ${ExecutionLogPackage::class.simpleName} ")
        }
