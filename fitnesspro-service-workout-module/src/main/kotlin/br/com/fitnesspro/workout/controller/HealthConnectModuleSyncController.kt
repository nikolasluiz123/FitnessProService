...
import br.com.fitnesspro.workout.service.health.HealthConnectModuleSyncService
...
@Tag(name = "Health Connect Module Sync Controller", description = "Controle de Importação e Exportação de Dados do Health Connect")
class HealthConnectModuleSyncController(
    private val healthConnectModuleSyncService: HealthConnectModuleSyncService
) {

    @GetMapping(EndPointsV1.SYNC_IMPORT_HEALTH_CONNECT)
    @Transactional(timeout = Timeouts.OPERATION_HIGH_TIMEOUT, rollbackFor = [Exception::class])
    @SecurityRequirement(name = "Bearer Authentication")
    fun import(@RequestParam filter: String, @RequestParam pageInfos: String, request: HttpServletRequest): ResponseEntity<ValidatedImportationServiceResponse<ValidatedHealthConnectModuleSyncDTO>> {
        ...
        val value = healthConnectModuleSyncService.getImportationData(queryFilter, commonPageInfos)
    ...
    @PostMapping(EndPointsV1.SYNC_EXPORT_HEALTH_CONNECT)
    @Transactional(timeout = Timeouts.OPERATION_HIGH_TIMEOUT, rollbackFor = [Exception::class])
    @SecurityRequirement(name = "Bearer Authentication")
    fun export(@RequestBody @Valid syncDTO: ValidatedHealthConnectModuleSyncDTO, request: HttpServletRequest): ResponseEntity<ValidatedExportationServiceResponse> {
        healthConnectModuleSyncService.saveExportedData(syncDTO)
